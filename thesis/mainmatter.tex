...
The foundations of QGSP are finally presented in Chapter \ref{ch:QGSP}, with discussions and a few applications with both toy \red{(verify if I still managed to create the toy examples)} and real world datasets.
The thesis closes with concluding remarks and future works, in Chapter \ref{ch:conclusion}. The Appendix \ref{ch:AppendixA} leads the reader through the demonstration of a relation between systems of quaternion and complex linear equations \red{(reescrever?)}, useful for a central theorem presented in Chapter \ref{ch:reviewQuat}.
...
The exploration of the quaternion multiplication rules gives rise to a useful property, related to complex numbers and elements from the quaternion canonical basis. In particular, the product between $\qj$ and any number $x = a + b \qi \in \mathbb{C}$ satisfies $\qj x = \overline{x} \qj$, since
\begin{equation}
\label{eq:commutej}
\begin{aligned}
...
\end{equation}
\red{(em GSP eu usei l1-norm, mas no gspx eu usei l2-norm. Quais as consequencias?)}
...
One may define a quaternion graph signal as being composed by the four last columns of weather data in UK towns (see Table \ref{tab:02}). That is, each sample corresponds to the quaternion with components $(1, \qi, \qj, \qk)$ respectively equal to humidity, pressure, temperature and wind speed in the given town. The slace disparity between components is removed by scaling each column to be within range $[0, 1]$. A way of visualizing the graph signal is to plot the graph with each node color defined by its quaternion sample: the components $(\qi, \qj, \qk)$ dictate the RGB color channels, whereas the scalar part of the quaternion defined the node alpha value (opacity). Figure \ref{fig:uk_qgsp_graphsig} depicts this form of signal visualization.

\begin{figure}
	\centering
	\includegraphics[width=0.3\linewidth]{thesis/Figures/uk_qgsp_graphsig.pdf}
	\caption{Quaternion graph signal, visualized in the vertex domain.}
...
\end{equation}
and the signal frequency components are shown in Figure \ref{fig:uk_qgsp_spectrumsig}. Figure \ref{fig:uk_qgsp_spectrumsig_abs} provides the reader with a more concise view, making clear this signal has energy concentrated in middle frequencies.

\begin{figure*}
\centering
\subfloat[\label{fig:uk_qgsp_spectrumsig}]{
\includegraphics[width=0.7\linewidth]{thesis/Figures/uk_qgsp_spectrumsig.pdf}
}\\
\subfloat[\label{fig:uk_qgsp_spectrumsig_abs}]{
\includegraphics[width=0.7\linewidth]{thesis/Figures/uk_qgsp_spectrumsig_abs.pdf}
}
\caption{(a) Spectrum of the graph signal, depicted with (a) splitted components and (b) only with the absolute value.}
\end{figure*}


\section{On the existence of a class of graphs with diagonalizable adjacency matrix}
...
At this point, an important observation must be made. It is known that every Hermitian complex matrix is diagonalizable by a unitary matrix. In other words, if $\mathbf{M} \in \mathbb{C}^{n \times n}$ and $\mathbf{M} = \mathbf{M}^H$, then there exists a complex matrix $\mathbf{U}$ and a diagonal matrix $\mathbf{\Gamma}$ so that $\mathbf{M} = \mathbf{U} \mathbf{\Gamma} \mathbf{U}^{-1}$ and $\mathbf{U}^{-1} = \mathbf{U}^H$. However, despite the fact one can guarantee that a Hermitian QGSO has a Hermitian complex adjoint matrix, \textit{it is not true that its eigenvector matrix is unitary}.
...
0 & 0 & 0 & 7 -4 \qi -8 \qj -9 \qk & 7 + 6 \qi + 7 \qj + 8 \qk\\ 
0 & 0 & 0 & 0 & 8 + 3 \qi + 7 \qk \\ 
0 & 0 & 0 & 0 & 7 \qi + 7 \qj + \qk\\ 
7 + 4\qi + 8 \qj + 9 \qk & 0 & 0 & 0 & 7 + 2\qi + 2 \qj + 1 \qk\\ 
7 -6 \qi -7 \qj -8 \qk & 8 -3 \qi -7 \qk & -7 \qi -7 \qj -1 \qk & 7 -2 \qi -2 \qj -1 \qk & 0
\end{pmatrix}.
\end{equation}
...
\rchi_A = 
\begin{pmatrix}
  0 &  0 &  0 &  7 - 4  \qi &  7 + 6  \qi &  0 &  0 &  0 & -8 - 9 \qi & 7 + 8 \qi \\
 0 &  0 &  0 &  0 &  8 + 3  \qi &  0 &  0 &  0 &  0 & 0 + 7 \qi \\
 0 &  0 &  0 &  0 &  0 + 7  \qi &  0 &  0 &  0 &  0 & 7 + 1 \qi \\
 7 + 4  \qi &  0 &  0 &  0 &  7 + 2  \qi &  8 + 9  \qi &  0 &  0 &  0 & 2 + 1 \qi \\
 7 - 6  \qi &  8 - 3  \qi &  0 - 7  \qi &  7 - 2  \qi &  0 & -7 - 8  \qi &  0 - 7  \qi & -7 - 1  \qi & -2 - 1 \qi & 0 \\
-0 & -0 & -0 &  8 - 9  \qi & -7 + 8  \qi &  0 &  0 &  0 &  7 + 4 \qi & 7 - 6 \qi \\
-0 & -0 & -0 & -0 & -0 + 7  \qi &  0 &  0 &  0 &  0 & 8 - 3 \qi \\
-0 & -0 & -0 & -0 & -7 + 1  \qi &  0 &  0 &  0 &  0 & 0 - 7 \qi \\
-8 + 9  \qi & -0 & -0 & -0 & -2 + 1  \qi &  7 - 4  \qi &  0 &  0 &  0 & 7 - 2 \qi \\
 7 - 8  \qi & -0 - 7  \qi &  7 - 1  \qi &  2 - 1  \qi & -0 &  7 + 6  \qi &  8 + 3  \qi &  0 + 7  \qi &  7 + 2 \qi & 0
\end{pmatrix}.
\end{equation}

Since the matrix $\rchi_A$ is complex-valued and Hermitian, it is diagonalizable by a unitary matrix $\mathbf{\Phi}$.

\begin{equation}
\begin{aligned}
\small
\mathbf{V} = \left(\begin{matrix}
0.4 & 0.610 +0.06\qj +0.04\qk & 0 \\
0.300 -0.1\qi -0.09\qj -0.1\qk & -0.130 +0.16\qi +0.14\qj +0.17\qk & -0.300 +0.06\qi -0.16\qj +0.61\qk \\
0.290 -0.1\qi -0.09\qj -0.1\qk & -0.130 +0.16\qi +0.13\qj +0.17\qk & 0.300 -0.06\qi +0.17\qj -0.63\qk \\
0.190 -0.21\qi -0.19\qj -0.2\qk & -0.440 -0.22\qi -0.26\qj -0.27\qk & 0 \\
0.000 +0.39\qi +0.38\qj +0.4\qk & -0.140 -0.1\qi -0.11\qj -0.13\qk & 0 \\
\end{matrix}\right. \\
\small
\left.\begin{matrix}
-0.410 -0.270\qj -0.120\qk & 0.460-0.030\qj -0.040\qk \\
0.150 +0.09\qi +0.290\qj +0.320\qk & 0.280 +0.05\qi +0.040\qj +0.020\qk \\
0.150 +0.09\qi +0.280\qj +0.310\qk & 0.280 +0.05\qi +0.040\qj +0.020\qk \\
0.140 -0.16\qi -0.210\qj -0.410\qk & 0.400 +0.15\qi +0.140\qj +0.100\qk \\
0.240 -0.03\qi +0.100\qj -0.010\qk & 0.430 -0.24\qi -0.300\qj -0.280\qk \\
\end{matrix}\right)
\end{aligned}
\end{equation}
...
\subsection{A more general sufficient condition}
