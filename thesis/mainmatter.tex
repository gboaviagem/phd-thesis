...
This thesis is organized as follows. Chapter \ref{ch:fundamentos} brings a diverse set of concepts and results from the literature, providing the reader with a broad understanding of the quaternion algebra, quaternion and fractional transforms, and GSP, so as to follow easily the discussions to come.
...
The thesis closes with concluding remarks and future works, in Chapter \ref{ch:conclusion}, and the Appendix \ref{ch:AppendixA}, demonstrating a relation between systems of quaternion and complex linear equations, useful for a central theorem presented in Chapter\ref{ch:fundamentos}.

\section{Contributions}
The contributions present in this thesis are:
...
This work has so far published two papers in international journals and one in a global conference. The paper \emph{``Eigenstructure and fractionalization of the quaternion discrete Fourier transform''}, published in Optik, contains the results of Chapter \ref{ch:FrQDFT}. The paper \emph{``The Cosine Number Transform: A Graph Signal Processing Approach''}, presented in the 2019 edition of GlobalSIP, was created during short attempts to further generalize GSP to other algebraic structures, in particular to graphs with edge weights in finite fields. However, as it has not evolved to more substantial findings and thus, although it stems from this work, it is not featured in this thesis.
...
\chapter{On quaternion algebra and its Fourier transform}
\label{ch:reviewQuat}
...
Only a decade later, in 1843, while walking by the roads in Dublin towards the RIA, ``an electric circuit seemed to close, and a spark flashed forth'', as he would say. He had conceived the four-dimensional structure required to the desired algebra, creating the quaternions. Moved by excitement, he craved on the stone below Broome Bridge, in Cabra (Dublin), the equations that define the relations between the canonical basis elements of quaternions\footnote{Close to the original site of the inscriptions, the RIA placed a commemorative plaque in 1958, with the same writings.}. This creation, made possible by an insight in 1843, permeates most of this work.
...
\chapter{Fractional and graph Fourier transforms}

\section{Fractional transforms}

\section{Fundamentals of graph signal processing (from jcis paper)}
...
\begin{figure}
	\centering
	\includegraphics[width=0.5\linewidth]{Figures/signal_duher_graph_2.pdf}
...
\begin{figure}
	\centering
	\subfloat[]{
		\includegraphics[width=0.35\linewidth]{Figures/example_graph_01_math.pdf}
	}~
	\subfloat[\label{fig:example_graphs_b}]{
		\includegraphics[width=0.35\linewidth]{Figures/example_graph_02_math.pdf}
	}
...
\begin{figure}
	...
	\subfloat[\label{fig:k_hop_neighborhood_01}]{
		\includegraphics[width=0.45\linewidth]{Figures/k_hop_neighborhood_01.pdf}
	}~
	%	\qquad
	\subfloat[\label{fig:k_hop_neighborhood_02}]{
		\includegraphics[width=0.45\linewidth]{Figures/k_hop_neighborhood_02.pdf}
	}
	%\hspace{0.2cm}%
...
\section{GSP\textsubscript{A}: graphs and algebraic signal processing}
\label{sec:DSPG}
...
%{\color{red}The representation given by (\ref{eq:filter_poly}) for LSI graph filters indicates how knowing the eigenstructure of the shift operator positively impacts filter design. This is not an available information for networks affected by stochastic changes, and to address such a problem Kruzick and Moura have studied optimal filter design criteria on random graphs
%, by investigating deterministic approximations of the empirical spectral distribution of the random shift operators
%\cite{kruzick2018filter}.}
...
\begin{figure}
	\centering
	\includegraphics[width=0.7\linewidth]{Figures/graph_frequency_EN.pdf}
