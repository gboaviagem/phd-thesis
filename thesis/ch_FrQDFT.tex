...
% \newcommand{\modification}[1]{{\color{red}#1}}

% %% The amssymb package provides various useful mathematical symbols
% \usepackage[english]{babel}
% \usepackage{mathtools}
% \usepackage{booktabs}
% \usepackage{amsmath,amssymb,amsfonts,color,enumerate}
...
% % Renaming Theorem and Proof:
% %\newtheorem{teorema}{Theorem}
% %\renewenvironment{proof}{{\itshape Proof:}}{}

% %\newtheorem{lema}{Lema}
% %\newtheorem{corolario}{Corol\'ario}[theorem]
% \usepackage{subfig}
...
\chapter{On the fractionalization of the quaternion discrete Fourier transform}
...
%The kernel may have a single axis, e.~g. $ e^{-\qmu (\omega x + \nu y)} = e^{-\qmu (\omega x)} e^{-\qmu (\nu y)}$, being placed at the left or right side of the signal, or even.
...
% & Left & Right & Sandwich \\
%\midrule
%Single-axis & $ e^{-\qmu (\omega x + \nu y)} f(\cdot)$ & $ f(\cdot) e^{-\qmu (\omega x + \nu y)}$& \\
%Dual-axis & & & \\ 
%Factored & & & \\
%\bottomrule
%\end{tabular}
...
The results of Theorem \ref{th:01} are summarized in Table \ref{tab:01}. \modification{
...
%it follows \cite{candan2011} that
%\begin{equation}
%%\label{key}
%\text{2D-QDFT}\{ \mathbf{e}^{(\qmu)} \mathbf{e}^{(\qnu)^T} \} = (\lambda_{\qmu} \lambda_{\qnu}) \ \mathbf{e}^{(\qmu)} \mathbf{e}^{(\qnu)^T}.
%\end{equation}
}
...
\end{cases}
\end{equation}
with \modification{$0 <  \gamma \leq 2 $}.

The tent map parameters were set to $ s_0 = 0{.}3 $ and $ \gamma = 1{.}8 $\modification{, for no particular reason other than obeying the range of values for chaotic behaviour}. The proposed encryption algorithm was used on the image in Fig. \ref{fig:dice}, what yielded the ciphered output in Fig. \ref{fig:ciphered01}.
...
The quaternions form an associative division ring invented by William Hamilton in \modification{1843} \cite{hamilton1848xi}, as the first non-commutative algebra in history. It consists of the set  $ \mathbb{H} $ of numbers $ q = a + b \qi + c \qj + d \qk $, with $ a,b,c,d \in \mathbb{R} $ and $ \qi^2 = \qj^2 = \qk^2 = -1 $, along with usual multiplication and addition. The product between imaginary units $ \qi,\qj,\qk $, however, follow the rule in Fig. \ref{fig:quatmult}, e.~g. $ \qk \qi = \qj $ and $ \qk \qj = -\qi $. As a result, quaternion multiplication is non-commutative.

The real and vector parts of a quaternion $ q = a + b \qi + c \qj + d \qk $ are defined as $ {\cal S}(q) = a $ and $ {\cal V}(q) = b \qi + c \qj + d \qk $. A quaternion $ v $ is said to be \textit{pure} if and only if $ {\cal S}(v) = 0 $; it is a \textit{unit quaternion} whenever $ |v| \overset{\Delta}{=} \sqrt{a^2 + b^2 + c^2 + d^2} = 1 $. The conjugation of $ q $ is $ \bar{q} \overset{\Delta}{=} {\cal S}(q) - {\cal V}(q) $. There is an Euler form for quaternions, similar to the known complex case, given by
\begin{equation}
\label{eq:euler}
